...
export class MarkerService {
  ...
  constructor(private _http: HttpClient, private _router: Router, private _modalService: NzModalService) { }
  ...
  createMarker(data: TCreateMarkerBody): Observable<TMarker>{
    ...
    return this._http.post<TMarkerDTO>(MARKERS, body).pipe(map(markerTransformer), catchError(this._handleError))
  }
  ...
  getMarkerById(_id: string):Observable<TMarker> {
    return this._http.get<TMarkerDTO>(MARKER(_id.toString())).pipe(map(markerTransformer), catchError(this._handleError))
  }

  patchMarkerById(_id: string, data: TPatchMarkerBody): Observable<TPatchMarker>{
    const body = patchMarkerBody(data);
    return this._http.patch<TPatchMarkerDTO>(MARKER(_id.toString()), body).pipe(map(patchMarkerBody), catchError(this._handleError))
  }
  ...
  private _handleError = (error: HttpErrorResponse) => {
    ...
    return throwError(() => {this.errorMessageModal()});
  };
